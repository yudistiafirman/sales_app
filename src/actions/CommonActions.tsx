...
export const getLocationCoordinates = async (
  ...
  latitude: number,
  distance?: string
) => customRequest(BrikApiCommon.getLocationCoordinates(longitude, latitude, distance), 'GET');
...
export const projectByUserGetAction = async (search?: string) =>
  customRequest(BrikApiCommon.getProjectByUser(search), 'GET', undefined, true);
export const projectGetOneById = async (projectId: string) =>
  customRequest(BrikApiCommon.oneGetProject(projectId), 'GET', undefined, true);
...
export const getAddressSuggestion = async (search?: string, page?: number) =>
  customRequest(BrikApiCommon.addressSuggestion(search, page), 'GET', undefined, true);
...
export const getProjectDetail = async (companyId?: string) =>
  customRequest(BrikApiCommon.getProjectDetail(companyId), 'GET', undefined, true);

export const getProjectIndivualDetail = async (projectId: string) =>
  customRequest(BrikApiCommon.getProjectDetailIndividual(projectId), 'GET', undefined, true);

export const updateBillingAddress = async (projectId: string, payload: any) =>
  customRequest(BrikApiCommon.updateBillingAddress(projectId), 'PUT', payload, true);

export const updateLocationAddress = async (projectId: string, payload: any) =>
  customRequest(BrikApiCommon.updateLocationAddress(projectId), 'PUT', payload, true);
