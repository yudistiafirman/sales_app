...
import { StyleProp, ViewStyle, View, Image, StyleSheet, DeviceEventEmitter } from 'react-native';
...
import { resetAllStepperFocused, updateDataVisitation } from '@/redux/reducers/VisitationReducer';
...
function Preview({ style }: { style?: StyleProp<ViewStyle> }) {
  ...
  const onArrivedDriver = async () => {
    try {
      ...
      const responseUpdateDeliveryOrder = await updateDeliveryOrder(
        payload,
        operationTempData?.deliveryOrderId || operationData?.projectDetails?.deliveryOrderId
      );
  ...
  const savePhoto = () => {
    ...
    let photoType =
      photoNameParts && photoNameParts.length > 0 ? photoNameParts[photoNameParts.length - 1] : '';
    ...
    if (photo) {
      localFile = {
        ...
        attachType: navigateTo === ENTRY_TYPE.DRIVER ? operationAddedStep : undefined,
      };
    } else if (picker) {
      localFile = {
        file: {
    ...
    switch (navigateTo) {
      case CREATE_VISITATION:
        ...
        navigation.goBack();
        navigation.dispatch(StackActions.replace(navigateTo, { existingVisitation }));
        return;
      case PO:
        dispatch({
          type: 'addImages',
  ...
  return (
    <View style={[_style, styles.parent]}>
      <View style={styles.container}>
        {photo && <Image source={{ uri: `file:${photo}` }} style={styles.image} />}
        {picker && picker.type === 'application/pdf' && (
          <Pdf style={styles.image} source={{ uri: picker?.uri }} />
        )}
        {picker && (picker.type === 'image/jpeg' || picker.type === 'image/png') && (
      ...
      <View style={styles.conButton}>
        ...
        <View style={styles.buttonTwo}>
          <BButtonPrimary title="Lanjut" onPress={savePhoto} rightIcon={ContinueIcon} />
        </View>
      </View>
    </View>
  );
