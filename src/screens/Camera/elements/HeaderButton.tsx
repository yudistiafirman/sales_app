...
import { StyleProp, ViewStyle, View, StyleSheet, ListRenderItem } from 'react-native';
...
function HeaderButton({
  ...
  const cameraHeaderButtonValue = [
    ...
    },
    {
      ...
      iconName: enableLowBoost ? 'lightbulb-on-outline' : 'lightbulb-off-outline',
    },
  ];

  const renderItem: ListRenderItem<{ onPress: () => void; iconName: string }> = React.useCallback(
    ({ item }) => (
      <>
        <BSpacer size="small" />
        <TouchableOpacity style={styles.photoIconContainer} onPress={item.onPress}>
          <MaterialCommunityIcons name={item.iconName} color={colors.white} size={resScale(20)} />
        </TouchableOpacity>
      </>
    ),
    []
