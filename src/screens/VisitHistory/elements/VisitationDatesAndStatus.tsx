...
function VisitationDatesAndStatus({
  ...
  const getLocalBookingDate = () => {
    let date = '-';
    let day = null;
    date = `${new Date(bookingDate).getDate()} ${new Date(bookingDate).toLocaleString(locale(), {
      month: 'short',
  ...
  const getLocalFinishDate = () => {
    let date = '-';
    date = `${new Date(finishDate).getDate()} ${new Date(finishDate).toLocaleString(locale(), {
      month: 'short',
  ...
  const renderCompBaseOnStatus = () => {
    ...
    if (status === 'SPH' && quatationId !== undefined) {
      return (
        <BTouchableText onPress={getOneOrder} textStyle={styles.touchableText} title="Lihat SPH" />
      );
    }
    return (
      <BText style={[styles.date, { marginRight: layout.pad.md }]}>
        ...
        <BChip backgroundColor={getBackgroundColor()}>
          <BText
            style={[
              styles.status,
              {
                color: status === 'REJECTED' ? colors.white : colors.text.darker,
              },
            ]}>
            {getStatus()}
          </BText>
