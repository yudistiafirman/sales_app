...
function PriceList() {
  ...
  const renderHeaderRight = React.useCallback(() => {
    ...
    return <BTouchableText onPress={() => setVisibleTnc(true)} title="Ketentuan" />;
  }, [fromVisitation]);
  ...
  const goToLocation = () => {
    if (!fromVisitation) {
      ...
      const coordinate = {
        ...
        latitude: Number(lat),
        formattedAddress: searchFormattedAddress.length > 0 ? searchFormattedAddress : '',
      };
        ...
        <BTabSections
          ...
          renderScene={() => (
            <ProductList
              ...
              isError={state.matches('getProduct.categoriesLoaded.errorGettingProducts')}
