...
export default function SelectedPIC({
  ...
  const [createScheduleState, stateUpdate] = React.useContext(
    CreateScheduleContext
  );

  const inputsData: Input[] = React.useMemo(() => {
    const values = flatListData.map((item) => {
      if (item.id === createScheduleState?.selectedSPH?.id) {
        return {
          ...item,
          isSelected: true,
        };
  ...
  }, [flatListData, createScheduleState]);
    ...
    <View style={style.container}>
      <View>
        <View style={{ minHeight: resScale(75) }}>
          ...
          <BVisitationCard
            item={{
              name: createScheduleState?.selectedCompany?.name,
              location: createScheduleState?.selectedCompany?.location,
            }}
      ...
      <BButtonPrimary
        disable={!createScheduleState?.selectedSPH ? true : false}
        title="Lanjut"
        onPress={() => {
