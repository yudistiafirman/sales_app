...
import { SafeAreaView, StyleSheet, View, Text, TouchableOpacity, Platform, Share } from 'react-native';
import {
	StackActions,
	useNavigation,
	useRoute,
...
function ListProduct(
	...
	let displayName = '';
	if (item.ReqProduct) {
		displayName = `${item?.ReqProduct?.product?.category?.parent
			? item?.ReqProduct?.product?.category?.parent?.name + ' '
			: ''
			}${item?.ReqProduct?.product?.displayName} ${item?.ReqProduct?.product?.category
				? item?.ReqProduct?.product?.category?.name
				: ''
			}`;
	} else if (item.Product) {
		displayName = `${item?.Product?.category?.parent?.name
...
const TransactionDetail = () => {
	...
	const shareFunc = async (url?: string) => {
		try {
			...
			await Share.share({
				...
				message: `Link PDF SPH ${data?.Company?.name
					? data?.Company.name
					: data?.Pic?.name}, ${url.replace(
						/\s/g,
						'%20'
					)}`,
			});
	...
	console.log(data?.QuotationLetterFiles, "data")
