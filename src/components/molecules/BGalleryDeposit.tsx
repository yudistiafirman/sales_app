...
export default function BGalleryDeposit({ picts, nominal, createdAt }: BGalleryDepositType) {
  return (
      ...
      {picts && picts.length > 1 && (
        <View
          ...
          {picts[1]?.isFromPicker ? (
            <>
              {picts[1]?.file?.type === 'image/jpeg' || picts[1]?.file?.type === 'image/png' ? (
                <Image style={style.flexFull} source={picts[1]?.file} />
              ) : (
                <Pdf source={{ uri: picts[1]?.file?.uri }} style={style.flexFull} page={1} />
              )}
            </>
          ) : (
            <Image style={style.flexFull} source={picts[1]?.file} />
