...
import { View, StyleSheet, TouchableOpacity, Image, ListRenderItem } from 'react-native';
...
export default function BGallery({ picts, addMorePict, removePict }: BGalleryType) {
  const renderItem: ListRenderItem<LocalFileType> = React.useCallback(
    ({ item, index }) => (
      <View style={[style.container, item.attachType ? { marginBottom: layout.pad.ml * 2 } : {}]}>
        ...
        {item?.isFromPicker ? (
          <>
            {item?.file?.type === 'image/jpeg' || item?.file?.type === 'image/png' ? (
              ...
              <Pdf source={{ uri: item?.file?.uri }} style={style.imageStyle} page={1} />
            )}
          </>
        ) : (
          <Image source={item?.file} style={style.imageStyle} />
        ...
        {item?.type === 'GALLERY' && removePict && (
          <TouchableOpacity
            ...
            <AntDesign name="close" size={resScale(15)} color={colors.white} />
          </TouchableOpacity>
        )}
        {item.attachType && (
