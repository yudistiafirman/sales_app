  ...
  createMachine(
    {
      ...
      schema: {
        events: {} as
          ...
          | { type:'onCloseModal'}
          | { type:'setDataType',value:'SPHDATA'| 'DEPOSITDATA' | 'SCHEDULEDATA'}
          | {type:'onRefresh'}
          | {type:'onEndReached'},
        ...
        services: {} as {
          ...
          GetPurchaseOrderList:{
            data:{currentPage:number,totalItems:number,totalPages:number,data:CreatedPurchaseOrderListResponse[]}
          }
        },
        context:{} as {
          searchValue:string;
    ...
    },
    {
      ...
      guards: {
        ...
        isDataTypePurchaseOrder:(context)=>{
          return context.dataType === 'DEPOSITDATA' || context.dataType === 'SCHEDULEDATA'
        },
        isGettingPurchaseOrder:(context)=> {
          return context.dataType === 'DEPOSITDATA' || context.dataType === 'SCHEDULEDATA' && context.page <= context.totalPage
        }
      },
      services: {
        ...
        GetPurchaseOrderList:async (context)=> {
          try {
            ...
            const response = await getConfirmedPurchaseOrder(page.toString(),size.toString(),searchValue,productPo)
            return response.data.data
          } catch (error) {
            throw new Error(error)
          }
