...
import { getCreatedSphDocuments, postPurchaseOrder } from '@/actions/OrderActions';
import {
    ...
    },
    {
      services: {
        ...
        getSphDocument: async context => {
          try {
            const sphId = context.choosenSphDataFromModal.QuotationRequests[0].QuotationLetter.id;
        ...
        uploadPhoto: async context => {
          try {
            ...
            const response = await uploadFileImage(photoFiles, 'Purchase Order');
        ...
        uploadFiles: async context => {
          try {
            const docsToUpload = context.files
              ...
              .map(v => v.value);
            const response = await uploadFileImage(docsToUpload, 'Purchase Order');
            return response.data.data;
          } catch (error) {
            throw new Error(error);
          }
        ...
        postPo: async context => {
          try {
            ...
            if (context.checked === 'first') {
              if (context.fiveToSix !== '') finalPayload.fiveToSix = Number(context.fiveToSix);
              if (context.lessThanFive !== '')
                finalPayload.lessThanFive = Number(context.lessThanFive);
            }
      ...
      guards: {
        ...
        isChoosenCustomerCompany: (context, event) => context.customerType === 'COMPANY',
        ...
        needUploadFiles: context => {
          const hasFileNotUploadedBefore = context.files.filter(v => v.projectDocId === null);
          ...
          return hasUploadedNpwpBefore.length > 0 || JSON.stringify(hasUploadedKtpBefore) === '{}';
        },
      ...
      actions: {
        ...
        assignFilesToPayload: assign((context, event) => {
          ...
          event.data.forEach(photo => {
            ...
            for (const documentId in context.files) {
              if (Object.prototype.hasOwnProperty.call(context.files, documentId)) {
                const photoData = context.files[documentId].value;
                if (photoData) {
                  if (photoData.name === photoName || photoData.name === photoNamee) {
        ...
        assignDocument: assign((_context, event) => {
          const requiredFileInput = event.data?.QuotationRequest?.ProjectDocs?.map(
            (val: ProjectDocs) => ({
        ...
        setSelectedChoosenProduct: assign((context, event) => {
          ...
          const isExisted = context.selectedProducts.findIndex(val => val.id === event.value.id);
          if (isExisted === -1) {
            newSelectedProduct = [...context.selectedProducts, event.value];
          } else {
            newSelectedProduct = context.selectedProducts.filter(val => val.id !== event.value.id);
          }
          return {
        ...
        assignPoPayload: assign(context => {
          const { QuotationLetter } = context.choosenSphDataFromModal.QuotationRequests[0];
          ...
          return {
            ...
            postPoPayload: {
              ...
              poNumber: context.customerType === 'INDIVIDU' ? '' : context.poNumber,
              poProducts:
        ...
        assignNewQuantity: assign((context, event) => {
          ...
          const newQuotationRequest = [...context.choosenSphDataFromModal.QuotationRequests][0];
