        ...
        assignTransactionsDataToContext: assign((context, event) => {
          if (event.data.data.length > 0) {
            ...
            const newTypeData = context.routes.map(item => ({
              ...
              title: item.title,
              totalItems:
                context.selectedType === item.title ? event.data.totalItems : item.totalItems,
              chipPosition: 'bottom',
            }));
            return {
              loadTransaction: false,
        ...
        getTransactions: async (_context, _event) => {
          try {
            ...
            if (_context.selectedType === 'PO') {
              ...
              response = await getAllDeposits(_context.page.toString(), _context.size.toString());
              response = response.data;
            } else if (_context.selectedType === 'Jadwal') {
              response = await getAllSchedules(_context.page.toString(), _context.size.toString());
              response = response.data;
